...
export default function Company() {
    ...
    if (saved) {
      try {
        ...
        const fixed = parsed.map(c => ({
          ...c,
          theme: c.theme?.id ? c.theme : Object.values(THEMES)[0],
          permissions: c.permissions || generateDefaultPermissions(c.role || 'employee'),
          companyName: c.companyName || c.username,
          email: c.email || '',
          phone: c.phone || '',
          address: c.address || '',
  ...
  // Save companies whenever changed
  useEffect(() => {
  ...
  const openModal = (company = null) => {
    ...
    if (company) {
      ...
      setFormData({
        ...
        permissions: JSON.parse(JSON.stringify(company.permissions)),
...
// // Helper to count enabled permissions for table display
// const countEnabledPermissions = (permissions) => {
//   if (!permissions) return { total: 0, enabled: 0 };
...
//       Object.values(submodule).forEach(value => {
//         total++;
//         if (value) enabled++;
//       });
//     });
...
// // Mock logged-in user data - Replace with your actual auth context
...
//   const openCreateModal = (company = null) => {
//     // If trying to edit a superadmin and logged-in user is not superadmin
//     if (company && company.role === 'superadmin' && loggedInUser.role !== 'superadmin') {
//       alert('You do not have permission to edit Super Admin accounts.');
//       return;
//     }
...
//   const handleCompanySubmit = (companyData, isEditing) => {
//     if (isEditing) {
...
//     if (activeFilter === 'active') return matchesSearch && company.status === 'active';
//     if (activeFilter === 'inactive') return matchesSearch && company.status === 'inactive';
//     if (activeFilter === 'admin') return matchesSearch && (company.role === 'admin' || company.role === 'superadmin');
//     if (activeFilter === 'superadmin') return matchesSearch && company.role === 'superadmin';
//     return matchesSearch;
//   });
...
//       {/* SUMMARY CARDS */}
//       <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
//         {/* ... Keep the existing summary cards code ... */}
//         {/* (Copy the summary cards from your original code) */}
//       </div>
...
//               <tbody className="divide-y divide-gray-200">
//                 {filteredCompanies.map((company) => {
//                   const permStats = countEnabledPermissions(company.permissions);
//                   return (
//                     <tr key={company.id} className="hover:bg-gray-50/50 transition-colors">
//                       <td className="px-6 py-4">
//                         <div className="flex items-center gap-3">
...
//                           <div className="text-sm text-gray-600">
//                             <span className="font-medium">{permStats.enabled}</span>/{permStats.total} enabled
//                           </div>
//                           <div className="w-32 h-2 bg-gray-200 rounded-full overflow-hidden">
//                             <div 
//                               className="h-full bg-green-500 transition-all duration-300"
//                               style={{ width: `${(permStats.enabled / permStats.total) * 100}%` }}
//                             />
//                           </div>
//                           <div className="text-xs text-gray-500">
//                             {Math.round((permStats.enabled / permStats.total) * 100)}%
//                           </div>
//                         </div>
//                       </td>
//                       <td className="px-6 py-4">
